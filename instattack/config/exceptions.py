...
class ConfigSchemaError(Exception):
    ...
    def humanize_errors(self, errors):
        ...
        for error in tuples:
            ...
            label_formatter = config.Colors.MED_GRAY
            formatted_attr = config.Colors.BLACK.format(bold=True)(error[0])
            formatted_error = config.Colors.ALT_RED.format(bold=True)(error[1].title())
